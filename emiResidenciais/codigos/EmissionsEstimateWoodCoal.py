...
def emissionEstimateWoodCoal(WoodCoalDf,DataPath,OutPath):
    ...
    emiWoodEmission = (WoodCoalDf['Consumo_lenha[ton/ano]'].values[:,np.newaxis] * emissionFactor.loc[:, 'Lenha'].values[np.newaxis, :]) / 1000
    emiWoodEmission = (WoodCoalDf['Consumo_Carvao[ton/ano]'].values[:,np.newaxis] * emissionFactor.loc[:, 'Carvao'].values[np.newaxis, :]) / 1000
    
    woodEmission[emissionFactor['Poluentes']] = emiWoodEmission
    coalEmission[emissionFactor['Poluentes']] = emiWoodEmission
